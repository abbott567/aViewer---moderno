...
public partial class MainWindow : Window
{
    ...
    private AccessibilityNode? _lastExternalTarget;
    ...
    private void ShowResult(AccessibilityInspectionSnapshot snapshot, string source)
    {
        ...
        var externalTarget = snapshot.RootFor(ActiveApi) ?? snapshot.UiaRoot ?? snapshot.Ia2Root ?? snapshot.MsaaRoot;
        if (externalTarget is not null && externalTarget.ProcessId != Environment.ProcessId)
        {
            _lastExternalTarget = externalTarget;
        }
    ...
    private async void CompleteTree_Click(object sender, RoutedEventArgs e)
    {
        var startingNode =
            _selectedNode is { ProcessId: not 0 } selected && selected.ProcessId != Environment.ProcessId
                ? selected
                : _lastExternalTarget;

        if (startingNode is null || startingNode.ProcessId == Environment.ProcessId)
        {
            StatusText.Text = L("InspectExternalElementFirst");
            return;
        }
        ...
        _inspectionTimer.Stop();
        InspectPointerButton.IsChecked = false;
        PointerToolbarButton.IsChecked = false;
        InspectFocusButton.IsChecked = false;
        FocusToolbarButton.IsChecked = false;

        var processId = startingNode.ProcessId;
        var current = startingNode;
        AccessibilityInspectionSnapshot? bestSnapshot = null;
        AccessibilityNode? bestRoot = null;

        StatusText.Text = L("LoadingCompleteTree");

        try
        {
            // Walk towards the application root. Each parent is loaded with a generous
            // descendant depth, so the final same-process ancestor contains the full app tree.
            for (var level = 0; level < 64; level++)
            {
                var candidateSnapshot = await Task.Run(() =>
                    _inspector.InspectParent(current, ActiveApi, 64));
                var candidateRoot = candidateSnapshot.RootFor(ActiveApi)
                    ?? candidateSnapshot.UiaRoot
                    ?? candidateSnapshot.Ia2Root
                    ?? candidateSnapshot.MsaaRoot;

                if (candidateRoot is null || candidateRoot.ProcessId != processId)
                {
                    break;
                }

                if (string.Equals(candidateRoot.Id, current.Id, StringComparison.Ordinal))
                {
                    break;
                }
            ...
            {
                // The inspected element may already be the application root. Preserve the
                // current snapshot rather than falling back to keyboard focus.
                bestSnapshot = _snapshot;
                bestRoot = startingNode;
            }

            if (bestSnapshot is null || bestRoot is null)
            {
                StatusText.Text = L("CompleteTreeUnavailable");
                return;
            }

            ShowResult(bestSnapshot, L("CompleteTreeSource"));
            StatusText.Text = LF("CompleteTreeLoaded", bestRoot.ProcessName, CountNodes(bestRoot));
        }
        catch (Exception ex)
        {
            StatusText.Text = LF("CompleteTreeFailed", ex.Message);
        }
    }
